...
async function example3_generateResponse() {
  ...
  const response = await provider.generateResponse({
    ...
    systemPrompt: 'You are a helpful AI assistant that explains complex topics simply.',
    temperature: 0.7,
    maxTokens: 500,
  });
...
async function main() {
  try {
    ...
    if (process.env.ANTHROPIC_API_KEY || process.env.OPENAI_API_KEY) {
      ...
      console.log('Set ANTHROPIC_API_KEY or OPENAI_API_KEY to run all examples');
    }
  } catch (error) {
