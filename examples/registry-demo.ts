...
import { RegistryManager, SQLiteBackend, ArtifactType } from '../src/registry/index';
...
async function main() {
  log('PCL REGISTRY DEMO', 'Demonstrating enterprise-grade artifact management');

  // ═════════════════════════════════════════════════════════════════════════════
  // 1. SETUP
  // ═════════════════════════════════════════════════════════════════════════════

  log('STEP 1: Initialize Registry', 'Setting up SQLite backend with in-memory database');
  ...
  log('STEP 2: Create Artifacts', 'Creating personas with automatic slug generation');
  ...
  if (programmingPersonas.ok) {
    logSuccess(`Found ${programmingPersonas.value.length} programming personas`);
  ...
  if (searchResult.ok) {
    logSuccess(`Found ${searchResult.value.length} results for "python expert"`);
  ...
  if (pythonExpert.ok) {
    const updateResult = await registry.update(pythonExpert.value.id, {
      metadata: {
        ...
        description: 'Updated: Expert Python developer with modern best practices',
      },
      stats: {
    ...
    if (updateResult.ok) {
      ...
      console.log(`  New description: ${updateResult.value.metadata.description}`);
