...
export const KEYWORDS = new Set([
  ...
  'persona', 'team', 'workflow', 'skill', 'constraint',
  'type', 'interface', 'enum', 'fn', 'let', 'const',
  'var', 'import', 'export', 'from', 'module', 'package',
  
  // Control flow
  'if', 'else', 'match', 'for', 'while', 'loop',
  'break', 'continue', 'return', 'yield', 'await',
  'try', 'catch', 'finally', 'throw', 'then',
  
  // Modifiers
  'pub', 'priv', 'mut', 'async', 'static', 'abstract',
  'override', 'final', 'lazy', 'inline', 'readonly',
  
  // Type keywords
  'String', 'Int', 'Float', 'Bool', 'Void', 'Never',
  'Any', 'Unknown', 'Self', 'Option', 'Result',
  'Array', 'Map', 'Set', 'Tuple',
  
  // Composition
  'merge', 'primary', 'chain', 'parallel', 'spawn',
  'delegate', 'isolate', 'compose', 'extends',
  'implements', 'with', 'as', 'requires',
  ...
  // PCL-specific
  'skills', 'constraints', 'tags', 'members', 'steps',
  'input', 'output', 'timeout', 'retry', 'fallback',
  'quorum', 'conflict', 'weights', 'topic',
  'depth', 'verbosity', 'tone', 'context', 'lang',
  'trace', 'audit', 'times', 'until',
  
  // Merge modes
  'Primary', 'Consensus', 'Majority', 'Dissent',
  'Compare', 'Append', 'Debate', 'Chain', 'Vote',
...
export const RESERVED = new Set([
  'class', 'struct', 'trait', 'impl', 'macro', 'unsafe',
  'extern', 'super', 'this', 'delete', 'sizeof',
  'alignof', 'offsetof', 'asm', 'volatile',
]);
...
export class Lexer {
  ...
  private scanToken(): Token | null {
    ...
    switch (c) {
      ...
      case '@': return this.makeToken(TokenType.AT, c);
      case '#': return this.makeToken(TokenType.HASH, c);
      ...
      case '*':
        if (this.match('*')) {
          if (this.match('=')) return this.makeToken(TokenType.STAR_STAR_EQ, '**=');
      ...
      case '!':
        if (this.match('=')) {
          if (this.match('=')) return this.makeToken(TokenType.BANG_EQ_EQ, '!==');
      ...
      case '<':
        ...
        if (this.match('-')) {
          if (this.match('>')) return this.makeToken(TokenType.LT_MINUS_GT, '<->');
          // Backtrack
          this.pos--;
          this.column--;
        }
      ...
      case '&':
        if (this.match('&')) {
          if (this.match('=')) return this.makeToken(TokenType.AMP_AMP_EQ, '&&=');
      ...
      case '|':
        if (this.match('|')) {
          if (this.match('=')) return this.makeToken(TokenType.PIPE_PIPE_EQ, '||=');
      ...
      case '?':
        if (this.match('?')) {
          if (this.match('=')) return this.makeToken(TokenType.QUESTION_QUESTION_EQ, '??=');
          return this.makeToken(TokenType.QUESTION_QUESTION, '??');
        }
        if (this.match('.')) return this.makeToken(TokenType.QUESTION_DOT, '?.');
        if (this.match('[')) return this.makeToken(TokenType.QUESTION_LBRACKET, '?[');
        return this.makeToken(TokenType.QUESTION, c);
      
      case '.':
        if (this.match('.')) {
          if (this.match('.')) return this.makeToken(TokenType.DOT_DOT_DOT, '...');
  ...
  private scanWhitespace(): Token {
    ...
    return this.makeToken(TokenType.WHITESPACE, this.input.slice(this.startPos, this.pos));
  }
  ...
  private scanNumber(startsWithDot: boolean, firstChar?: string): Token {
    ...
    if (!startsWithDot && this.peek() === '.' && this.isDigit(this.peekNext())) {
    ...
    const durationMatch = /^(ms|[smhd])$/.exec(this.input.slice(this.pos, this.pos + 2));
    if (durationMatch) {
    ...
    const cleanValue = raw.replace(/_/g, '');
    return {
      ...this.makeToken(isFloat ? TokenType.NUMBER_FLOAT : TokenType.NUMBER_INT, cleanValue),
      raw,
    };
  }
  ...
  private scanIdentifier(): Token {
    ...
    if (RESERVED.has(value)) {
      return this.errorToken(`Reserved word '${value}' cannot be used as identifier`);
    }
  ...
  private errorToken(message: string): Token {
    const token = this.makeToken(TokenType.ERROR, this.input.slice(this.startPos, this.pos));
...
export function isIdentifier(token: Token): boolean {
  return token.type === TokenType.IDENTIFIER || token.type === TokenType.PERSONA_ID;
}
...
export function isBinaryOperator(token: Token): boolean {
  return [
    ...
    TokenType.LT, TokenType.GT, TokenType.LT_EQ, TokenType.GT_EQ, TokenType.LT_EQ_GT,
    TokenType.AMP_AMP, TokenType.PIPE_PIPE, TokenType.QUESTION_QUESTION,
...
export function isAssignmentOperator(token: Token): boolean {
  return [
    ...
    TokenType.AMP_AMP_EQ, TokenType.PIPE_PIPE_EQ, TokenType.QUESTION_QUESTION_EQ,
  ].includes(token.type);
}
...
export function getOperatorPrecedence(token: Token): number {
  switch (token.type) {
    ...
    case TokenType.QUESTION_QUESTION_EQ:
      return 2;
    case TokenType.QUESTION: return 3; // Ternary
    case TokenType.QUESTION_QUESTION: return 4;
    case TokenType.PIPE_PIPE: return 5;
    case TokenType.AMP_AMP: return 6;
    case TokenType.PIPE: return 7;
    case TokenType.CARET: return 8;
    case TokenType.AMP: return 9;
