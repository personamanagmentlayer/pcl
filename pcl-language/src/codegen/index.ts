...
class PromptGenerator {
  ...
  generate(persona: AST.PersonaDeclaration): string {
    ...
    if (props.methods.length > 0) {
      ...
      for (const method of props.methods) {
        this.bullet(`${method.name}: ${method.description || 'Available action'}`);
      }
    }
  ...
  generateTeam(
    ...
    if (config.quorum) {
      this.line(`Quorum Required: ${config.quorum.required}/${config.quorum.total}`);
    }
  ...
  private generateMergeInstructions(mode: string): void {
    switch (mode) {
      case 'primary':
        this.line('The primary lead makes final decisions. Other members provide input and suggestions.');
        break;
      case 'consensus':
        this.line('All team members must reach agreement. Discuss until consensus is achieved.');
        break;
      case 'majority':
        this.line('Decisions are made by majority vote. Each member has equal weight.');
        break;
      case 'debate':
        this.line('Members should present different perspectives. Explore pros and cons thoroughly.');
        break;
      case 'append':
        this.line('Each member contributes their unique perspective. All contributions are included.');
        break;
      case 'weighted':
        this.line('Members have different influence weights. Higher-weighted opinions carry more impact.');
        break;
      case 'chain':
        this.line('Process sequentially. Each member builds on the previous member\'s output.');
        break;
      default:
...
class JSONGenerator {
  ...
  private addPersona(target: any, decl: AST.PersonaDeclaration): void {
    ...
    for (const member of decl.body.members) {
      switch (member.kind) {
        ...
        case 'SkillBlock': {
          const block = member as AST.SkillBlock;
          persona.skills = block.items.map(item => {
            if (item.kind === 'StringSkill') {
        ...
        case 'ConstraintBlock': {
          const block = member as AST.ConstraintBlock;
          persona.constraints = block.items.map(item => {
            if (item.kind === 'StringConstraint') {
              return (item as { value: string }).value;
            }
            return null;
        ...
        case 'TagBlock': {
          const block = member as AST.TagBlock;
          persona.tags = block.items.map(item => {
            if (item.kind === 'StringTag') {
              return (item as { value: string }).value;
            }
        ...
        case 'MethodDeclaration': {
          ...
          persona.methods[method.name.name] = {
            ...
            parameters: method.parameters.map(p => ({
              name: p.name.kind === 'Identifier' ? (p.name as AST.Identifier).name : null,
              optional: p.optional,
            })),
          };
          break;
    ...
    for (const member of decl.body.members) {
      switch (member.kind) {
        case 'TeamMembersDeclaration': {
          ...
          team.members = membersDecl.members.map(ref => this.personaRefToString(ref));
          break;
        }
        ...
        case 'TeamConflictDeclaration': {
          ...
          team.conflictOrder = conflictDecl.order.map(ref => this.personaRefToString(ref));
          break;
        }
      }
    }
    ...
    for (const member of decl.body.members) {
      switch (member.kind) {
        case 'WorkflowInputDeclaration': {
          workflow.input = this.typeNodeToString((member as AST.WorkflowInputDeclaration).type);
          break;
        }
        
        case 'WorkflowOutputDeclaration': {
          workflow.output = this.typeNodeToString((member as AST.WorkflowOutputDeclaration).type);
          break;
        }
        
        case 'WorkflowStepsDeclaration': {
          workflow.steps = this.workflowExprToObject((member as AST.WorkflowStepsDeclaration).steps);
          break;
        }
  ...
  private addSkill(target: any, decl: AST.SkillDeclaration): void {
    ...
    for (const member of decl.body.members) {
      if (member.kind === 'SkillItemsDeclaration') {
        skill.items = (member as AST.SkillItemsDeclaration).items.map(i => i.value);
      }
      if (member.kind === 'SkillCategoryDeclaration') {
        skill.category = (member as AST.SkillCategoryDeclaration).category.value;
      }
    }
    ...
    switch (expr.kind) {
      ...
      case 'ArrayExpression':
        return (expr as AST.ArrayExpression).elements
          .filter((e): e is AST.Expression => e !== null && e.kind !== 'SpreadElement')
      ...
      case 'ObjectExpression':
        ...
        for (const prop of (expr as AST.ObjectExpression).properties) {
          if (prop.kind === 'ObjectKeyValueProperty') {
            ...
            const key = kv.key.kind === 'Identifier'
  ...
  private typeNodeToString(type: AST.TypeNode): string {
    switch (type.kind) {
      ...
      case 'UnionType':
        return (type as AST.UnionType).types.map(t => this.typeNodeToString(t)).join(' | ');
      case 'IntersectionType':
        return (type as AST.IntersectionType).types.map(t => this.typeNodeToString(t)).join(' & ');
      case 'ArrayType':
  ...
  private workflowExprToObject(expr: AST.WorkflowExpression): any {
    switch (expr.kind) {
      case 'WorkflowSequenceExpr':
        return {
          type: 'sequence',
          steps: (expr as AST.WorkflowSequenceExpr).steps.map(s => this.workflowExprToObject(s)),
        };
      case 'WorkflowParallelExpr':
        return {
          type: 'parallel',
          branches: (expr as AST.WorkflowParallelExpr).branches.map(b => this.workflowExprToObject(b)),
        };
      case 'WorkflowChoiceExpr':
        return {
          type: 'choice',
          branches: (expr as AST.WorkflowChoiceExpr).branches.map(b => this.workflowExprToObject(b)),
        };
      ...
      case 'WorkflowMergeExpr':
        return {
          type: 'merge',
          mode: (expr as AST.WorkflowMergeExpr).mode.kind === 'SimpleMergeMode'
...
export function generateTypeScript(
  ...
  const opts = { ...DEFAULT_OPTIONS, ...options, target: 'typescript' as const };
...
class TypeScriptGenerator {
  ...
  generate(program: AST.Program): string {
    ...
    // Imports
    this.line("import { createRuntime, createPersona, createTeam } from '@pcl/runtime';");
    this.blank();
  ...
  private generateTeam(decl: AST.TeamDeclaration): void {
    ...
    this.indent();
    this.line(`const members = ${name}Config.members.map(m => personas.get(m)).filter(Boolean);`);
  ...
  private generateWorkflow(decl: AST.WorkflowDeclaration): void {
    ...
    for (const member of decl.body.members) {
      if (member.kind === 'WorkflowInputDeclaration') {
        this.line(`// input: ${this.typeToString((member as AST.WorkflowInputDeclaration).type)}`);
      }
      if (member.kind === 'WorkflowOutputDeclaration') {
        this.line(`// output: ${this.typeToString((member as AST.WorkflowOutputDeclaration).type)}`);
      }
      if (member.kind === 'WorkflowStepsDeclaration') {
        this.line(`steps: ${JSON.stringify(this.workflowExprToArray((member as AST.WorkflowStepsDeclaration).steps))},`);
      }
    }
  ...
  private generateInterface(decl: AST.InterfaceDeclaration): void {
    ...
    for (const member of decl.members) {
      if (member.kind === 'PropertySignature') {
        ...
        const propName = typeof prop.name === 'string' ? prop.name : (prop.name as AST.Identifier).name;
        const opt = prop.optional ? '?' : '';
        const readonly = prop.readonly ? 'readonly ' : '';
        this.line(`${readonly}${propName}${opt}: ${this.typeToString(prop.type)};`);
      }
    }
  ...
  private generateFunction(decl: AST.FunctionDeclaration): void {
    ...
    const params = decl.parameters.map(p => {
      const pName = p.name.kind === 'Identifier' ? (p.name as AST.Identifier).name : '_';
      const pType = p.type ? this.typeToString(p.type) : 'any';
      const opt = p.optional ? '?' : '';
      return `${pName}${opt}: ${pType}`;
    }).join(', ');
    
    const returnType = decl.returnType ? this.typeToString(decl.returnType) : 'void';
  ...
  private generateVariable(decl: AST.VariableDeclaration): void {
    ...
    for (const declarator of decl.declarations) {
      if (declarator.id.kind === 'Identifier') {
        ...
        const type = declarator.type ? `: ${this.typeToString(declarator.type)}` : '';
        const init = declarator.init ? ` = ${this.exprToString(declarator.init)}` : '';
  ...
  private typeToString(type: AST.TypeNode): string {
    switch (type.kind) {
      ...
      case 'UnionType':
        return (type as AST.UnionType).types.map(t => this.typeToString(t)).join(' | ');
      case 'IntersectionType':
        return (type as AST.IntersectionType).types.map(t => this.typeToString(t)).join(' & ');
      case 'ArrayType':
      ...
      case 'FunctionType': {
        ...
        const params = func.parameters.map(p => {
          const name = p.name.kind === 'Identifier' ? (p.name as AST.Identifier).name : '_';
      ...
      case 'LiteralType': {
        ...
        if (lit.literal.kind === 'StringLiteral') return `'${(lit.literal as AST.StringLiteral).value}'`;
        if (lit.literal.kind === 'NumberLiteral') return String((lit.literal as AST.NumberLiteral).value);
        if (lit.literal.kind === 'BooleanLiteral') return String((lit.literal as AST.BooleanLiteral).value);
        return 'unknown';
      }
      default:
        return 'unknown';
  ...
  private workflowExprToArray(expr: AST.WorkflowExpression): string[] {
    switch (expr.kind) {
      case 'WorkflowSequenceExpr':
        return (expr as AST.WorkflowSequenceExpr).steps.flatMap(s => this.workflowExprToArray(s));
      case 'WorkflowParallelExpr':
        return [`parallel:${(expr as AST.WorkflowParallelExpr).branches.length}`];
      case 'WorkflowPersonaRef': {
...
class MarkdownGenerator {
  ...
  private generatePersonaDoc(decl: AST.PersonaDeclaration): void {
    ...
    for (const member of decl.body.members) {
      switch (member.kind) {
        ...
        case 'SkillBlock':
          for (const item of (member as AST.SkillBlock).items) {
            if (item.kind === 'StringSkill') skills.push((item as { value: string }).value);
          }
          break;
        case 'ConstraintBlock':
          for (const item of (member as AST.ConstraintBlock).items) {
            if (item.kind === 'StringConstraint') constraints.push((item as { value: string }).value);
          }
          break;
        case 'TagBlock':
          for (const item of (member as AST.TagBlock).items) {
            if (item.kind === 'StringTag') tags.push((item as { value: string }).value);
            else if (item.kind === 'IdentifierTag') tags.push((item as { name: AST.Identifier }).name.name);
          }
          break;
        case 'MethodDeclaration': {
          const method = member as AST.MethodDeclaration;
    ...
    if (decl.extends.length > 0) {
      const exts = decl.extends.map(e => e.typeName.parts.map(p => p.name).join('::'));
    ...
    for (const member of decl.body.members) {
      switch (member.kind) {
        case 'TeamMembersDeclaration':
          ...
          break;
        case 'TeamPrimaryDeclaration':
          if ((member as AST.TeamPrimaryDeclaration).primary.ref.type === 'id') {
            primary = (member as AST.TeamPrimaryDeclaration).primary.ref.id.name;
          }
          break;
        case 'TeamMergeDeclaration':
          if ((member as AST.TeamMergeDeclaration).mode.kind === 'SimpleMergeMode') {
  ...
  private generateWorkflowDoc(decl: AST.WorkflowDeclaration): void {
    ...
    for (const member of decl.body.members) {
      switch (member.kind) {
        ...
        case 'WorkflowTimeoutDeclaration': {
          const timeout = member as AST.WorkflowTimeoutDeclaration;
          this.line(`**Timeout:** ${timeout.duration.value}${timeout.duration.unit}`);
          break;
        }
        case 'WorkflowRetryDeclaration': {
          const retry = member as AST.WorkflowRetryDeclaration;
          if (retry.config.kind === 'NumberLiteral') {
            this.line(`**Retries:** ${(retry.config as AST.NumberLiteral).value}`);
          }
          break;
        }
      }
    ...
    for (const member of decl.body.members) {
      if (member.kind === 'WorkflowStepsDeclaration') {
        this.h4('Flow');
        this.code('mermaid', this.generateMermaid((member as AST.WorkflowStepsDeclaration).steps));
        this.blank();
      }
    }
  ...
  private typeToString(type: AST.TypeNode): string {
    if (type.kind === 'TypeReference') {
      return (type as AST.TypeReference).typeName.parts.map(p => p.name).join('::');
    }
    return 'Unknown';
  }
