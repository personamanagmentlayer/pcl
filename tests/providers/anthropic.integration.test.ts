// ═══════════════════════════════════════════════════════════════════════════════
// PCL Runtime - Anthropic Provider Integration Tests
...
describeIfApiKey('AnthropicProvider Integration', () => {
  ...
  describe('generateResponse', () => {
    test('generates response with Claude', async () => {
    ...
    test('includes metadata with provider and model', async () => {
      ...
      expect(response.metadata?.model).toBe('claude-3-5-haiku-20241022');
      expect(response.metadata?.id).toBeTruthy(); // Message ID
    ...
    test('uses default model when not specified', async () => {
      ...
      expect(response.metadata?.model).toBe('claude-3-7-sonnet-20250219');
    });
  ...
  describe('Tool Calling', () => {
    test('supports tool calling', async () => {
      ...
      // Claude may or may not use the tool depending on the response
      // But the request should not throw errors
      expect(response).toBeDefined();
      expect(response.content).toBeTruthy();
    });

    test('parses tool calls when Claude uses them', async () => {
      const tools: Tool[] = [
        {
          name: 'calculate',
      ...
      // If Claude used the tool, should have toolCalls
  ...
  describe('Configuration', () => {
    ...
    test('sets default model', () => {
      provider.setDefaultModel('claude-3-5-haiku-20241022');
      const model = provider.getDefaultModel();
      expect(model).toBe('claude-3-5-haiku-20241022');
    });

    test('throws error for unsupported model', () => {
      expect(() => provider.setDefaultModel('gpt-4')).toThrow(
        "Model 'gpt-4' not supported by Anthropic provider"
      );
    });
  ...
  describe('Capabilities', () => {
    test('reports correct capabilities', () => {
      ...
      expect(provider.capabilities.maxContextWindow).toBe(200_000);
    });

    test('lists available Claude models', () => {
  ...
  describe('Edge Cases', () => {
    ...
    test('handles empty prompt', async () => {
      ...
      // Claude should handle empty prompts gracefully or throw a clear error
...
describe('AnthropicProvider (No API Key)', () => {
