  ...
  describe('Security Review Scenario', () => {
    ...
    it('should compile and load security review scenario', async () => {
      ...
      if (result.ok) {
        ...
        expect(runtime.getPersona('CODE_REVIEWER')?.getState().active).toBe(true);
      }
    });
  ...
  describe('Content Creation Scenario', () => {
    ...
    it('should handle content creation workflow', async () => {
      ...
      if (result.ok) {
        ...
        const writerResponse = await runtime.send('WRITER', 'Write a product description');
        expect(writerResponse.ok).toBe(true);
        
        // Team should be able to process
        const teamResponse = await runtime.sendToTeam('CONTENT_TEAM', 'Create blog post about AI');
    ...
    it('should process analysis workflow', async () => {
      ...
      if (result.ok) {
        ...
        if (parseResult.ok) {
          ...
          expect(config.personas.SUMMARIZER.skills).toContain('Executive summaries');
